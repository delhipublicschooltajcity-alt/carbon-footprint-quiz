...
const LS_KEY = "dps_tajcity_cf_v15";

/* ========= HARD FAIL-SAFE ========= */
window.addEventListener("error", (e) => {
  const msg = `❌ App error: ${e.message || e.error || "Unknown error"}`;
  const box = document.getElementById("submitStatus");
  if (box) box.textContent = msg;
  console.error(e);
});
window.addEventListener("unhandledrejection", (e) => {
  const msg = `❌ Promise error: ${e.reason || "Unknown error"}`;
  const box = document.getElementById("submitStatus");
  if (box) box.textContent = msg;
  console.error(e);
});
...
/* Badge banding based on CO2 total (family of 4) */
...
/* Habit footprint score (0 best → 100 worst) maps to CO2 total (tCO2e/year family of 4) */
...
/* Split total CO2 into areas with weights */
...
/* Footprint score: eco% -> footprint (0 best, 100 worst) */
...
function calcCO2Breakdown(overallFootprintScore, arenaFootprint) {
  ...
  const byAreaT = {};
  ARENAS.forEach((a) => {
    byAreaT[a] = Math.round((totalT * (raw[a] / sum)) * 10) / 10;
  });
...
function buildRecommendations(totalT, byAreaT) {
  ...
  const tips = {
    transport: [
      "Prefer school bus or a consistent carpool.",
      "Switch off engine while waiting near school.",
      "Keep tyre pressure and service on time.",
    ],
    home: [
      "Prefer inverter split AC if buying new.",
      "Use LEDs in most-used rooms.",
      "Reduce AC hours where possible.",
    ],
    devices: [
      "Switch off chargers/TV at night.",
      "Reduce background screens.",
    ],
    food: [
      "Plan portions to reduce food waste.",
      "Prefer local + seasonal vegetables.",
    ],
    waste: [
      "Segregate wet/dry daily (2 bins).",
...
/* Render results (NO marks anywhere) */
function renderResults() {
  // If something is missing, show error clearly
  if (!badgeTextEl || !whereYouAreEl || !co2TotalEl || !compareBarsEl || !arenaScoresEl || !recommendationsEl) {
    if (submitStatus) {
      submitStatus.textContent = "❌ Missing result UI elements (ID mismatch). Please ensure HTML IDs match app.js.";
    }
    return;
  }
...
async function submitToSheet() {
  ...
  if (submitStatus) submitStatus.textContent = "Saving to Google Sheet...";
  ...
  try {
    ...
    save();
    if (submitStatus) submitStatus.textContent = "✅ Saved. Refreshing leaderboard...";
  } catch (e) {
    state.submittedOnce = false;
    save();
    if (submitStatus) submitStatus.textContent = `❌ Save failed: ${e?.message || e}`;
  }
}
    ...
    all.forEach(row => {
      ...
      div.innerHTML = `
        ...
        <div>
          ...
          <div style="opacity:.85;font-size:.92em">${esc(row.className || "-")}</div>
        </div>
...
btnNext.onclick = async () => {
  ...
  if (state.arenaIndex >= ARENAS.length) {
    ...
    try {
      ...
      if (submitStatus) submitStatus.textContent = `❌ Render failed: ${e?.message || e}`;
      console.error(e);
    }
  } else {
    renderArenaPage();
